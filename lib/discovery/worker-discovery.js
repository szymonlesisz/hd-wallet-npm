...
var WorkerDiscovery = exports.WorkerDiscovery = function () {
    ...
    _createClass(WorkerDiscovery, [{
        ...
        value: function tryHDNode(xpub, network) {
            try {
                var node = _bitcoinjsLibZcash.HDNode.fromBase58(xpub, network);
                return node;
            } catch (e) {
                return e;
            }
        ...
        value: function discoverAccount(initial, xpub, network, segwit) {
            ...
            var out = new _outside.WorkerDiscoveryHandler(this.discoveryWorkerFactory, this.chain, sources, network);
        ...
        value: function monitorAccountActivity(initial, xpub, network, segwit) {
            ...
            var resNull = blockStream.concat(txNotifs).mapPromiseError(function (tx) {
                var doIt = false;
                if (tx === 'block') {
                    var unconfirmed = currentState.transactions.filter(function (t) {
                ...
                if (doIt) {
                    var out = new _outside.WorkerDiscoveryHandler(_this2.discoveryWorkerFactory, _this2.chain, sources, network);
